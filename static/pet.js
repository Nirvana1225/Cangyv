...
// 狸花猫 sleep
const CAT_SLEEP = (() => { const f = CAT_IDLE.map(r => r.split("")); for(let y=10;y<=11;y++){for(let x=0;x<32;x++){if(f[y][x]==="4")f[y][x]="1"}} return f.map(r=>r.join("")); })();

// 狸花猫 happy（眯眼）
const CAT_HAPPY = (() => { const f = CAT_IDLE.map(r => r.split("")); for(let y=10;y<=11;y++){for(let x=0;x<32;x++){if(f[y][x]==="4")f[y][x]="5"}} return f.map(r=>r.join("")); })();

// 狸花猫 scratch（伸爪子挠/抓人）
const CAT_SCRATCH = (() => { const f = CAT_IDLE.map(r => r.split("")); for(let x=8;x<=12;x++){f[x+14][x]="8"} return f.map(r=>r.join("")); })();

// 狸花猫 eat（低头吃）
const CAT_EAT = (() => { const f = CAT_IDLE.map(r => r.split("")); for(let y=14;y<=16;y++){for(let x=10;x<=22;x++){if(y>=15)f[y][x]="0"}} f[14][15]="6"; return f.map(r=>r.join("")); })();
